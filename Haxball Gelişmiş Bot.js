...
function checkTime() {
	...
	if (Math.abs(scores.time - scores.timeLimit) <= 0.01 && scores.timeLimit != 0) {
		...
		goldenGoal = true;
		room.sendChat("⚽ Altın Gol !");
	}
	if (Math.abs(drawTimeLimit * 60 - scores.time - 60) <= 0.01 && players.length > 2) {
		if (checkTimeVariable == false) {
			checkTimeVariable = true;
			setTimeout(() => { checkTimeVariable = false; }, 10);
			room.sendChat("⌛ Çekilişe 60 saniye kaldı !");
		}
	}
...
function endGame(winner) { // handles the end of a game : no stopGame function inside
	...
	if (winner == Team.RED) {
		streak++;
		room.sendChat("🔴 Kırmızı takım kazandı " + scores.red + "-" + scores.blue + " ! Kazanma Serisi : " + streak + " 🏆");
	}
	else if (winner == Team.BLUE) {
		streak = 1;
		room.sendChat("🔵 Mavi takım kazandı " + scores.blue + "-" + scores.red + " ! Kazanma Serisi : " + streak + " 🏆");
	}
	else {
		streak = 0;
		room.sendChat("💤 Beraberlik sınırına ulaşıldı! 💤");
    }
    room.sendChat("⭐ Topla oynama : 🔴 " + (Rposs*100).toPrecision(3).toString() + "% : " + (Bposs*100).toPrecision(3).toString() + "% 🔵");
...
function deactivateChooseMode() {
	...
	if (slowMode != 0) {
		slowMode = 0;
		room.sendChat("Yavaş mod kapatıldı.");
	}
	redCaptainChoice = "";
	blueCaptainChoice = "";
}
...
function handleInactivity() { // handles inactivity : players will be kicked after afkLimit
	...
	for (var i = 0; i < extendedP.length ; i++) {
		if (extendedP[i][eP.ACT] == 60 * (2/3 * afkLimit)) {
			room.sendChat("[PV] ⛔ @" + room.getPlayer(extendedP[i][eP.ID]).name + ", bir sonraki adımda hareket etmez veya mesaj göndermezseniz " + Math.floor(afkLimit / 6) + " saniye sonra kickleniceksin !", extendedP[i][eP.ID]);
		}
...
function choosePlayer() {
	...
	if (teamR.length <= teamB.length && teamR.length != 0) {
		room.sendChat("[PV] Bir oyuncu seçmek için verilen listeye numarasını girin veya 'üst', 'rastgele' veya 'alt' kullanın.", teamR[0].id);
		...
		room.sendChat("[PV] Bir oyuncu seçmek için verilen listeye numarasını girin veya 'üst', 'rastgele' veya 'alt' kullanın..", teamB[0].id);
...
room.onPlayerJoin = function(player) {
	...
	room.sendChat("[PV] 👋 Hoş Geldin " + player.name + " ", player.id);
	...
	if (["!help"].includes(message[0].toLowerCase())) {
		...
		if (players.length != 1 && player.team != Team.SPECTATORS) {
			if (player.team == Team.RED && streak > 0 && room.getScores() == null) {
				...
				room.sendChat("Takımdayken AFK gidemezsin !", player.id);
				return false;
			}
		}
...
room.onTeamGoal = function(team) {
	...
	if (lastPlayersTouched[0] != null && lastPlayersTouched[0].team == team) {
		if (lastPlayersTouched[1] != null && lastPlayersTouched[1].team == team) {
			room.sendChat("⚽ " + getTime(scores) + " Golü atan " + lastPlayersTouched[0].name + " ! Asisti yapan " + lastPlayersTouched[1].name + ". Şut hızı : " + ballSpeed.toPrecision(4).toString() + "km/h " + (team == Team.RED ? "🔴" : "🔵"));
			game.goals.push(new Goal(scores.time, team, lastPlayersTouched[0], lastPlayersTouched[1]));
		}
		else {
			room.sendChat("⚽ " + getTime(scores) + " Golü atan " + lastPlayersTouched[0].name + " ! Şut hızı : " + ballSpeed.toPrecision(4).toString() + "km/h " + (team == Team.RED ? "🔴" : "🔵"));
			game.goals.push(new Goal(scores.time, team, lastPlayersTouched[0], null));
		}
	}
	else {
		room.sendChat("😂 " + getTime(scores) + " Kendi kalesine attı " + lastPlayersTouched[0].name + " ! Şut hızı : " + ballSpeed.toPrecision(4).toString() + "km/h " + (team == Team.RED ? "🔴" : "🔵"));
